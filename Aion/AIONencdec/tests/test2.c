...
unsigned char obuf[] = "\x2f\x5d\x51\xf7\x01\xe9\xb4\x93\x4e\x51\x81\x3e\xaf\x3f\xdf\x99"
...
unsigned char cbuf[] =  "\x86\xFA\x1A\x1C\x07\xBD\xD8\x64\xCE\xEE\x59\x88\xCD\xA9\x1D\x06"
...
int main(void)
{
	unsigned int xor = 0,i,j,k;
	unsigned char buf[256],tbuf[25600],name[256];
	char *ptr1, *ptr2;
	BFdesc *fil, *fil2;
	...
	for(j=10600;j<10601;j++) {

		fil = bfopen(r,"quest_q1000.html",0);
		bfread(fil,tbuf,10622);
		bfclose(fil);

		xor = 0x81 & 0x3ff;
		for(i=1;i<10622;i++) {
			tbuf[i] ^= cbuf[xor+(i&0x1f)];
//			xor = tbuf[i];
		}
		sprintf(name,"xor%03d",j);
		printf("%s\n",name);
		fil = bfopen(w,name,0);
		bfwrite(fil,tbuf,1024);
		bfclose(fil);
	}
	...
	fil = bfopen(w,"cbinary",0);
	bfwrite(fil,cbuf,sizeof(cbuf)-1);
	bfclose(fil);

	name[8]=0;
	name[20]=0;
	putchar('\t');
	for(i=0;i<(sizeof(obuf)-1) >> 2;i++) {
#if 0
		for(j=7;j>=0;j--) {
			name[7-j] = 0x30+((obuf[i] >> j) & 1);
		}
		for(j=7;j>=0;j--) {
			name[12+7-j] = 0x30+(((~obuf[i]) >> j) & 1);
		}
//		printf("%s %X %+0.3d   %03u\n",name,obuf[i],obuf[i],(uint8_t)obuf[i]);
		printf("%2u: %s %2X %+.3d   %s %2X %+.3d\n",i%32,name,(uint8_t)obuf[i],(int8_t)obuf[i],name+12,(uint8_t)(~obuf[i]),(int8_t)(~obuf[i]));
#endif
		if (i && (i%8 == 0))
			printf("\n\t");
		printf("0x%08X, ",*((uint32_t *)obuf+i));
	}
	putchar('\n');
	putchar('\n');
	putchar('\n');
	...
	for(i=0;i<(sizeof(cbuf)-1) ;i++) {
		if (i && (i%16 == 0))
			printf("\n\t");
		printf("0x%02X, ",*((uint8_t *)cbuf+i));
	}
	putchar('\n');
	                            
	ptr1 = (char*)0xa0502334;
	ptr2 = ptr1 + 16;

	printf("%u\n", (ptr2 - ptr1));

	(uint32_t*)ptr1[0]=0x12345678;
