...
*/
				...
				case '\v': ((uint32_t*)(to+j))[0] = 0x42782326; ((uint32_t*)(to+j))[1] = 0x0000003B; j+=4; break;
				...
				default: if(from[i] < 0x1F || from[i] == 0x7F )
						return -1;
					else
						to[j] = from[i];
			}
...
int textconv_conv(const char *ocode, uint8_t **obuf, int *osiz, const char *icode, const uint8_t* ibuf, const int isiz, int dotrans, const int loud)
{
	...
	if(isiz > sl_lim) {	//input buffer may not be that long
	...
	outptr = (char*)tmpbufC;
	outrem = sl_lim;	//reserve for specific conditions
	
//	fwrite(ibuf,1,isiz,stdout);

//for stateful encodings
	if(iconv(cd, NULL, NULL, &outptr, &outrem) == (size_t)(-1)) EXCEPT;
