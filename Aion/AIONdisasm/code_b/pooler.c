...
static const char g_mall_err[]="Pooler failure: malloc call.\n";
...
void *PoolerAssign(POOLER* const pool, const uintptr_t siz)
{
	...
	if(siz > CHUNK_SIZ) {
		fprintf(stderr,"Pooler refuses to allocate more than %u MiB at once. Check your dat / txt / ddf files for errors.\nIf desperate, ask on forums.",CHUNK_SIZ/CHUNK_UNIT);
		EXCEPT;
	}
/*
...
void PoolerDestroy(POOLER * const pool)
{
	...
	if(!pool)
		return;
//	#ifndef NDEBUG
	printf("Total allocated by pooler \"%s\": %d\n", pool->name, pool->cnt_resrv*sizeof(void*) + pool->cnt_alloc*CHUNK_SIZ);
//	#endif
